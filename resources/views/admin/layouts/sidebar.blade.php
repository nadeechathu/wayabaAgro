<style>
    .sidebar, .menu-collapsed .aside .collapse {
        background-color : #001e1e !important;
    }
    .menu-item a,.menu-icon{
        color : #fff !important;
    }

    .menu-item .active,.menu-icon .active{
        color : gray !important;
    }

    .submenu a:hover{
        background-color : #001e1e !important;
    }
</style>
<!-- Page Aside-->
<aside class="aside bg-white sidebar">

    <div class="simplebar-wrapper">
        <div data-pixr-simplebar>
            <div class="pb-6">
                <!-- Mobile Logo-->
                <div class="d-flex d-xl-none justify-content-between align-items-center border-bottom aside-header">
                    <a class="navbar-brand lh-1 border-0 m-0 d-flex align-items-center" href="./index.html">
                        <div class="d-flex align-items-center">
                            <svg class="f-w-5 me-2 text-primary d-flex align-self-center lh-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 203.58 182"><path d="M101.66,41.34C94.54,58.53,88.89,72.13,84,83.78A21.2,21.2,0,0,1,69.76,96.41,94.86,94.86,0,0,0,26.61,122.3L81.12,0h41.6l55.07,123.15c-12-12.59-26.38-21.88-44.25-26.81a21.22,21.22,0,0,1-14.35-12.69c-4.71-11.35-10.3-24.86-17.53-42.31Z" fill="currentColor" fill-rule="evenodd" fill-opacity="0.5"/><path d="M0,182H29.76a21.3,21.3,0,0,0,18.56-10.33,63.27,63.27,0,0,1,106.94,0A21.3,21.3,0,0,0,173.82,182h29.76c-22.66-50.84-49.5-80.34-101.79-80.34S22.66,131.16,0,182Z" fill="currentColor" fill-rule="evenodd"/></svg>
                            <span class="fw-black text-uppercase tracking-wide fs-6 lh-1">{{config('app.name','GUI')}}</span>
                        </div>                    </a>
                    <i
                        class="ri-close-circle-line ri-lg close-menu text-muted transition-all text-primary-hover me-4 cursor-pointer"></i>
                </div>
                <!-- / Mobile Logo-->

                <ul class="list-unstyled mb-6">

                    <!-- Dashboard Menu Section-->
                    <li class="menu-item"><a class="d-flex align-items-center" href="{{route('home')}}">
                                <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" class="w-100">
                                        <rect fill-opacity=".5" fill="currentColor" x="3" y="3" width="7" height="7"></rect>
                                        <rect fill="currentColor" x="14" y="3" width="7" height="7"></rect>
                                        <rect fill-opacity=".5" fill="currentColor" x="14" y="14" width="7" height="7">
                                        </rect>
                                        <rect fill="currentColor" x="3" y="14" width="7" height="7"></rect>
                                    </svg>
                                </span>
                            <span class="menu-link">
                            </i> Dashboard

                            </span></a>
                        </li>
                    <!-- / Dashboard Menu Section-->

                @if($commonContent['sidebarSettings']->users == 1)
                    @if (Auth::user()->role_id == 1)

                     <!-- Users Menu Section-->
                     <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemUsers" aria-expanded="false"
                            aria-controls="collapseMenuItemUsers">
                            <span class="menu-icon">
                                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                    viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                       <path fill="currentColor" opacity=".5" d="M155.327,57.142c-51.531,0-93.454,44.45-93.454,99.086c0,54.636,41.923,99.086,93.454,99.086s93.455-44.45,93.455-99.086
                                           C248.782,101.592,206.859,57.142,155.327,57.142z"/>

                                       <path fill="currentColor" d="M367.798,71.321c-0.211,0-0.425,0.001-0.636,0.002c-21.626,0.179-41.826,9.31-56.878,25.713
                                           c-14.788,16.113-22.829,37.37-22.644,59.854c0.186,22.484,8.577,43.605,23.628,59.473c15.17,15.991,35.265,24.773,56.651,24.773
                                           c0.215,0,0.43-0.001,0.646-0.002c21.626-0.179,41.826-9.311,56.878-25.713c14.788-16.113,22.829-37.37,22.644-59.855
                                           C447.702,108.972,411.747,71.321,367.798,71.321z"/>

                                       <path fill="currentColor" d="M371.74,257.358h-7.76c-36.14,0-69.12,13.74-94.02,36.26c6.23,4.78,12.16,9.99,17.78,15.61
                                           c16.58,16.58,29.6,35.9,38.7,57.42c8.2,19.38,12.88,39.8,13.97,60.83H512v-29.87C512,320.278,449.08,257.358,371.74,257.358z"/>

                                       <path fill="currentColor" opacity=".5" d="M310.35,427.478c-2.83-45.59-25.94-85.69-60.43-111.39c-25.09-18.7-56.21-29.77-89.92-29.77h-9.34
                                           C67.45,286.319,0,353.768,0,436.978v17.88h310.65v-17.88C310.65,433.788,310.55,430.618,310.35,427.478z"/>

                               </svg>
                                </span>
                            <span class="menu-link"> Users</span></a>
                        <div class="collapse" id="collapseMenuItemUsers">
                            <ul class="submenu">
                            @if (Auth::user()->hasPermission('add_users'))
                                <li><a href="{{route('users.all')}}">Registered Users</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_customers'))
                                <li><a href="{{route('customers.all')}}">All Customers</a></li>
                            @endif

                            </ul>
                        </div>
                    </li>
                    <!-- / Users Menu Section-->


                    @endif
                @endif

                @if($commonContent['sidebarSettings']->products == 1)
                     <!-- product add start -->
                     <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemProducts" aria-expanded="false"
                            aria-controls="collapseMenuItemProducts">
                            <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                        <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                      <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                      </svg>
                                </span>
                            <span class="menu-link"> Products</span></a>
                        <div class="collapse" id="collapseMenuItemProducts">
                            <ul class="submenu">
                            @if (Auth::user()->hasPermission('add_products'))
                                    <li><a href="{{route('products.newProduct')}}">New Product</a></li>
                                @endif
                            @if (Auth::user()->hasPermission('view_products'))
                            <li><a href="{{route('products.all')}}">All Products</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_variants'))
                            <li><a href="{{route('products.variants.all')}}">Product Variants</a></li>
                            @endif
                            
                            </ul>
                        </div>
                    </li>
                <!-- product end -->
                @endif



             
                <!-- product add start -->
                <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                       data-bs-toggle="collapse" data-bs-target="#collapseMenuItemBrand" aria-expanded="false"
                       aria-controls="collapseMenuItemBrand">
                       <span class="menu-icon">
                               <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                   <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                   <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                 <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                 </svg>
                           </span>
                       <span class="menu-link">Brands</span></a>
                   <div class="collapse" id="collapseMenuItemBrand">
                       <ul class="submenu">
                  
                       <li><a href="{{route('brands.all_brands')}}">All Brands</a></li>
 
                     

                       </ul>
                   </div>
               </li>
           <!-- product end -->
          








                @if($commonContent['sidebarSettings']->inventory == 1)
                    <!-- product add start -->
                    <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                                data-bs-toggle="collapse" data-bs-target="#collapseMenuItemInventory" aria-expanded="false"
                                aria-controls="collapseMenuItemInventory">
                                <span class="menu-icon">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                            <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                          <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                          </svg>
                                    </span>
                                <span class="menu-link"> Inventory</span></a>
                            <div class="collapse" id="collapseMenuItemInventory">
                                <ul class="submenu">
                                @if (Auth::user()->hasPermission('view_inventory'))
                                <li><a href="{{route('inventory.all')}}">Available Inventory</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('view_inventory_history'))
                                <li><a href="{{route('inventory.history.product')}}">Inventory History</a></li>
                                @endif

                                </ul>
                            </div>
                        </li>
                    <!-- product end -->
                @endif

                @if($commonContent['sidebarSettings']->categories == 1)
                    @if (Auth::user()->hasPermission('view_categories'))
                    <!-- Categories Menu Section-->

                    <!-- Posts Menu Section-->
                    <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemCategories" aria-expanded="false"
                            aria-controls="collapseMenuItemCategories">
                            <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                        <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                      <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                      </svg>
                                </span>
                            <span class="menu-link"> Categories</span></a>
                        <div class="collapse" id="collapseMenuItemCategories">
                            <ul class="submenu">
                                @if (Auth::user()->hasPermission('add_categories'))
                                    <li><a href="{{route('categories.new')}}">New Category</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('view_categories'))
                                    <li><a href="{{route('categories.all')}}">Main Categories</a></li>
                                    <li><a href="{{route('subCategoriesL1.all')}}">Sub Categories (L1)</a></li>
                                    <li><a href="{{route('subCategoriesL2.all')}}">Sub Categories (L2)</a></li>
                                @endif
                            </ul>
                        </div>
                    </li>
                    <!-- / Categories Menu Section-->
                    @endif
                 @endif

                {{--@if($commonContent['sidebarSettings']->tags == 1)
                    @if (Auth::user()->hasPermission('view_tags'))
                    <!-- Tags Menu Section-->
                    <li class="menu-item"><a class="d-flex align-items-center collapsed" href="{{route('tags.all')}}">
                                <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                        <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                      <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                      </svg>
                                </span>
                            <span class="menu-link"> Tags</span></a>

                    </li>
                    <!-- / Tags Menu Section-->
                    @endif
                 @endif--}}

                @if($commonContent['sidebarSettings']->all_orders == 1)
                    @if (Auth::user()->hasPermission('view_orders'))
                     <!-- Orders Menu Section-->
                     <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemOrders" aria-expanded="false"
                            aria-controls="collapseMenuItemOrders">
                            <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                        <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                      <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                      </svg>
                                </span>
                            <span class="menu-link"> All Orders</span></a>
                        <div class="collapse" id="collapseMenuItemOrders">
                            <ul class="submenu">
                            @if (Auth::user()->hasPermission('view_orders'))
                            <li><a href="{{route('orders.all')}}">All Orders</a></li>
                            @endif
                            {{--@if (Auth::user()->hasPermission('invoices_and_packing_slips'))
                            <li><a href="{{route('orders.invoices')}}">Invoices & Packing Slips</a></li>
                            @endif--}}
                            @if (Auth::user()->hasPermission('cancel_orders'))
                            <li><a href="{{route('orders.cancellationApproval')}}">Cancellation Approvals</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_cancelled_orders'))
                            <li><a href="{{route('orders.cancelledOrders')}}">Cancelled Orders</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_order_status'))
                            <li><a href="{{route('orders.statuses')}}">Order Statuses</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('manual_inventory_reserve'))
                            <li><a href="{{route('order.notReserved')}}">Not Reserved Orders</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_quotations'))
                            <li><a href="{{route('orders.quotations.all')}}">Quotations</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_pre_orders'))
                            <li><a href="{{route('orders.preOrders.all')}}">Pre Orders</a></li>
                            @endif
                            </ul>
                        </div>
                    </li>
                    <!-- / orders Menu Section-->
                    @endif
                @endif

                @if($commonContent['sidebarSettings']->posts == 1)
                     <!-- Posts Menu Section-->
                     <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemPosts" aria-expanded="false"
                            aria-controls="collapseMenuItemPosts">
                            <span class="menu-icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                        <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                      <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                      </svg>
                                </span>
                            <span class="menu-link"> Posts</span></a>
                        <div class="collapse" id="collapseMenuItemPosts">
                            <ul class="submenu">
                                @if (Auth::user()->hasPermission('view_posts'))
                                    <li><a href="{{route('posts.all')}}">All Posts</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('add_posts'))
                                    <li><a href="{{route('posts.new')}}">New Post</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('approve_posts'))
                                    <li><a href="{{route('posts.approval')}}">Approve Posts</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('view_comments'))
                                    <li><a href="{{route('postComments.all')}}">Comments</a></li>
                                @endif
                            </ul>
                        </div>
                    </li>
                    <!-- / Posts Menu Section-->

                @endif
                @if($commonContent['sidebarSettings']->marketing == 1)
                       <!-- Posts Menu Section-->
                       <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                        data-bs-toggle="collapse" data-bs-target="#collapseMenuItemCoupons" aria-expanded="false"
                        aria-controls="collapseMenuItemCoupons">
                        <span class="menu-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z" />
                                    <path opacity="0.5" d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z" />
                                  <path d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z" />
                                  </svg>
                            </span>
                        <span class="menu-link">Marketing</span></a>
                    <div class="collapse" id="collapseMenuItemCoupons">
                        <ul class="submenu">
                            @if (Auth::user()->hasPermission('view_coupons'))
                                <li><a href="{{route('coupon.all')}}">All Coupons</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_advertisements'))
                            <li><a href="{{route('advertisements.all')}}">All Advertisements</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('view_promotions'))
                            <li><a href="{{route('promotions.all')}}">All Promotions</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('assign_promotions'))
                            <li><a href="{{route('promotions.assignUI')}}">Assign Promotions</a></li>
                            @endif
                        </ul>
                    </div>
                    </li>
                @endif


                @if($commonContent['sidebarSettings']->web_pages == 1)
                    <!-- Website settings Menu Section-->
                           <li class="menu-item">


                     <a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemPages" aria-expanded="false"
                            aria-controls="collapseMenuItemPages">
                                <span class="menu-icon">
                                    <svg enable-background="new 0 0 520 520" viewBox="0 0 520 520" xmlns="http://www.w3.org/2000/svg"><g><path fill="currentColor" d="m481.734 100.063h-158.331l-43.111-70.397c-2.727-4.452-7.571-7.166-12.792-7.166h-119.235c-21.099 0-38.265 17.166-38.265 38.266v65.51 229.184c0 21.1 17.166 38.266 38.265 38.266h261.735 71.734c21.1 0 38.266-17.166 38.266-38.266v-217.13c0-21.101-17.166-38.267-38.266-38.267z"/><path fill="currentColor" opacity=".5" d="m80 355.459v-229.184h-41.734c-21.1 0-38.266 17.166-38.266 38.266v294.693c0 21.1 17.166 38.266 38.266 38.266h333.469c21.1 0 38.266-17.166 38.266-38.266v-35.51h-261.736c-37.641.001-68.265-30.623-68.265-68.265z"/></g></svg>
                                </span>
                        <span class="menu-link"> Web Pages</span></a>
                        <div class="collapse" id="collapseMenuItemPages">
                            <ul class="submenu">
                                @if (Auth::user()->hasPermission('view_pages'))
                                    <li><a href="{{route('webpages.all')}}">All Added Pages</a></li>
                                @endif
                                @if (Auth::user()->hasPermission('add_pages'))
                                    <li><a href="{{route('webpages.new')}}">New Page</a></li>
                                @endif
                            </ul>
                        </div>
                    </li>
                    <!-- / Website settings Menu Section-->
                @endif

                @if($commonContent['sidebarSettings']->zones == 1)
                    <!-- zones Menu Section-->
                    <li class="menu-item">

                         <a class="d-flex align-items-center collapsed" href="#"
                                data-bs-toggle="collapse" data-bs-target="#collapseMenuItemZones" aria-expanded="false"
                                aria-controls="collapseMenuItemZones">
                                    <span class="menu-icon">
                                        <svg enable-background="new 0 0 520 520" viewBox="0 0 520 520" xmlns="http://www.w3.org/2000/svg"><g><path fill="currentColor" d="m481.734 100.063h-158.331l-43.111-70.397c-2.727-4.452-7.571-7.166-12.792-7.166h-119.235c-21.099 0-38.265 17.166-38.265 38.266v65.51 229.184c0 21.1 17.166 38.266 38.265 38.266h261.735 71.734c21.1 0 38.266-17.166 38.266-38.266v-217.13c0-21.101-17.166-38.267-38.266-38.267z"/><path fill="currentColor" opacity=".5" d="m80 355.459v-229.184h-41.734c-21.1 0-38.266 17.166-38.266 38.266v294.693c0 21.1 17.166 38.266 38.266 38.266h333.469c21.1 0 38.266-17.166 38.266-38.266v-35.51h-261.736c-37.641.001-68.265-30.623-68.265-68.265z"/></g></svg>
                                    </span>
                            <span class="menu-link"> Zones</span></a>
                            <div class="collapse" id="collapseMenuItemZones">
                                <ul class="submenu">
                                    @if (Auth::user()->hasPermission('view_zones'))
                                        <li><a href="{{route('zones.all_zones')}}">All Zones</a></li>
                                    @endif

                                </ul>
                            </div>
                        </li>
                        <!-- / zones Menu Section-->
                @endif

                

                
                @if($commonContent['sidebarSettings']->inquiries == 1)
                        <!-- inquiry Menu Section-->
                        <li class="menu-item">

                         <a class="d-flex align-items-center collapsed" href="#"
                                data-bs-toggle="collapse" data-bs-target="#collapseMenuItemInquiries" aria-expanded="false"
                                aria-controls="collapseMenuItemInquiries">
                                    <span class="menu-icon">
                                        <svg enable-background="new 0 0 520 520" viewBox="0 0 520 520" xmlns="http://www.w3.org/2000/svg"><g><path fill="currentColor" d="m481.734 100.063h-158.331l-43.111-70.397c-2.727-4.452-7.571-7.166-12.792-7.166h-119.235c-21.099 0-38.265 17.166-38.265 38.266v65.51 229.184c0 21.1 17.166 38.266 38.265 38.266h261.735 71.734c21.1 0 38.266-17.166 38.266-38.266v-217.13c0-21.101-17.166-38.267-38.266-38.267z"/><path fill="currentColor" opacity=".5" d="m80 355.459v-229.184h-41.734c-21.1 0-38.266 17.166-38.266 38.266v294.693c0 21.1 17.166 38.266 38.266 38.266h333.469c21.1 0 38.266-17.166 38.266-38.266v-35.51h-261.736c-37.641.001-68.265-30.623-68.265-68.265z"/></g></svg>
                                    </span>
                            <span class="menu-link"> Inquiries</span></a>
                            <div class="collapse" id="collapseMenuItemInquiries">
                                <ul class="submenu">
                                @if (Auth::user()->hasPermission('view_inquiries'))
                                <li><a href="{{route('inquiries.all')}}">All Inquiries</a></li>
                                @endif

                                </ul>
                            </div>
                        </li>
                        <!-- / inquiry Menu Section-->
                @endif

                    @if (Auth::user()->hasPermission('site_settings') or Auth::user()->hasPermission('view_permissions'))
                     <!-- Users Menu Section-->
                     <li class="menu-item"><a class="d-flex align-items-center collapsed" href="#"
                            data-bs-toggle="collapse" data-bs-target="#collapseMenuItemPermissions" aria-expanded="false"
                            aria-controls="collapseMenuItemPermissions">
                            <span class="menu-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                stroke-linecap="round" stroke-linejoin="round" class="feather feather-grid">
                                <rect x="3" y="3" width="7" height="7"></rect>
                                <rect x="14" y="3" width="7" height="7"></rect>
                                <rect x="14" y="14" width="7" height="7"></rect>
                                <rect x="3" y="14" width="7" height="7"></rect>
                            </svg>
                            </span>
                            <span class="menu-link">Settings</span></a>
                        <div class="collapse" id="collapseMenuItemPermissions">
                            <ul class="submenu">
                            @if (Auth::user()->hasPermission('view_permissions'))
                                <li><a href="{{route('permissions.addNew')}}">Permission Settings</a></li>
                            @endif
                            @if (Auth::user()->hasPermission('site_settings'))

                                <li><a href="{{route('settings.header')}}">Slider Settings</a></li>
                                <li><a href="{{route('settings.analytics')}}">Analytics Settings</a></li>
                                <li><a href="{{route('settings.siteSettings')}}">Site Settings</a></li>
                                {{--<li><a href="{{route('settings.templates')}}">Site Templates</a/li>--}}
                                <li><a href="{{route('settings.countrySettings')}}">Country Settings</a></li>
                                <li><a href="{{route('settings.emailSettings')}}">Email Settings</a></li>
                                <li><a href="{{route('settings.units')}}">Units Settings</a></li>
                                    <li><a href="{{route('settings.sidebar')}}">Sidebar Settings</a></li>
                            @endif


                            </ul>
                        </div>
                    </li>
                    <!-- / Users Menu Section-->
                    @endif

                </ul>
            </div>
        </div>
    </div>

</aside>    <!-- / Page Aside-->
