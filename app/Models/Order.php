...
class Order extends Model
{
    ...
    public static function placeOrder(Request $request){
            ...
            if($orderData != null){
                ...
                $tracking_number = "ORD".date('Ymd').$saved_order->id;
                ...
                foreach($lineItems as $lineItem){
                    ...
                    $item->unit_price = $lineItem['unit_price'];
                    $item->total_price = (int)$lineItem['quantity'] * (float)$lineItem['unit_price'];
        ...
        if($cart != null){
            ...
            foreach($cart as $orderItem){
                ...
                if($product != null and $productVariant != null){
                    ...
                    //promotions are not applied for the bulk orders
                    if($productVariant->bulk_quantity != null){
                        if($orderItem['qty'] >= $productVariant->bulk_quantity){
                            //bulk orders
                    ...
                    $productWeight = 0; //in kg

                    if($product->weight_unit == "g"){

                        $productWeight = $productVariant->weight / 1000;
                    }


                    $totalWeight = $totalWeight + ($orderItem['qty'] * $productWeight);

                    // $orderItem->discount = $itemDiscount;
                    array_push($orderItems,array('product_id' => $orderItem['product_id'], 'variant_id' => $orderItem['variant_id'], 'quantity' => $orderItem['qty'], 'unit_price' => $product_price, 'discount' => $itemDiscount));
                }
