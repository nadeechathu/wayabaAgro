...
class OrderController extends Controller
{
        ...
        try{
            ...
            if($hasPermission){
                ...
                if($item != null){
                    ...
                    if(Auth::user()){
                        //saving user log
                        UserLog::saveUserLog(Auth::user()->id, "Order item quantity edited","Order item quantity edited in order ".$order->tracking_number." in item ".$item->hamper_name);
                    } 
        ...
        try{
            ...
            if($hasPermission){
                ...
                if($address != null){
                        ...
                        if(Auth::user()){
                            //saving user log
                            UserLog::saveUserLog(Auth::user()->id, "Order address updated","Order ".$addressType." address updated in order ".$order->tracking_number);
                        }
                }
    ...
    public function removeOrderStatus($id){
        ...
        if($hasPermission){
            ...
            OrderStatus::where('id',$request->status_id)->delete();
    ...
    public function viewPreOrders(Request $request){

    

        $hasPermission = Auth::user()->hasPermission('view_pre_orders');
        ...
        if($hasPermission){
            ...
            $preOrders = PreOrder::getPreOrderForFilters($searchKey);

           
       
            return view('admin.orders.all_pre_orders',compact('preOrders','searchKey'));
    ...
    public function removePreOrder(Request $request)
    {
            ...
            $pre_order = PreOrder::find($request->pre_id);
    
            $pre_order->status = 0;
            $pre_order->save();      

            return back()->with('success', 'Pre Order removed successfully!');
        ...
        public function storeRefund(Request $request)
        {
            ...
            if( $inputPassword != null){
                ...
                $super_admin = User::where('is_super',1)->get()->first();
         
                $mached = Hash::check( $inputPassword, $super_admin->password);

                if($mached){

                
                $order = Order::where('id',$order_id)->update(['order_status'=>9]);

                // EmailSender::sendOrderStatusEmail($order_id);

                return back()->with('success' ,'order marked as refund' );
                ...
                return back()->with('error','Super Admin authorization required');
            }
        }
